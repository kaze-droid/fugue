...
pub struct DesktopEnv {
    ...
    smooth_mouse_pos: Vector2,  // Smoothed mouse position for lag effect
}
...
impl DesktopEnv {
    ...
    pub fn open_window(&mut self, hw: &mut VirtualHardware, app_type: AppType) {
        ...
        let pid = hw.spawn_process(title, cpu_cost, ram_cost, true, true);  // true for is_ui
            ...
            if rl.is_mouse_button_down(MouseButton::MOUSE_BUTTON_LEFT) {
                if ctx.window_idx < self.windows.len() {
                    ...
                    let drag_speed = if hw.target_fps < 60 {
                        0.1 + (hw.target_fps as f32 / 60.0) * 0.9  // 0.1 at 20fps, 1.0 at 60fps
                    } else {
                        1.0  // Full speed at 60fps
                    };
                    ...
                    // Lerp window position towards target (slower when lagging)
    ...
    fn draw_sys_monitor(&self, d: &mut RaylibDrawHandle, hw: &VirtualHardware, r: Rectangle) {
        ...
        for (i, load) in hw.cpu_load.iter().enumerate() {
            ...
            // Different colors based on RL scheduling mode (no pulsing)
            let color = if hw.rl_scheduler_enabled && hw.current_mindset == KernelMindset::Rescheduling {
                ...
                if *load > 0.8 { 
                    Color::new(180, 0, 0, 255)
                } else if *load > 0.5 { 
                    Color::new(200, 120, 0, 255)
                } else { 
    ...
    fn draw_taskbar(&self, d: &mut RaylibDrawHandle, hw: &VirtualHardware, sw: i32, sh: i32) {
        ...
        d.draw_text("|  [F1] Monitor  [F2] Terminal  [F3] Files  [F6] RL", 90, sh - 28, 20, CYBER_THEME.text_dim);

        // Show kernel status on the right
        ...
        if hw.rl_decision_flash > 0.0 {
            ...
            let flash_color = Color::new(0, 255, 255, flash_alpha); // Cyan flash
