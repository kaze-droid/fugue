...
impl DesktopEnv {
    ...
    fn draw_sys_monitor(&self, d: &mut RaylibDrawHandle, hw: &VirtualHardware, r: Rectangle) {
        ...
        // Helper: draw text with shadow for contrast
        let draw_label = |d: &mut RaylibDrawHandle, text: &str, px: i32, py: i32, size: i32| {
            d.draw_text(text, px + 1, py + 1, size, Color::BLACK);  // Shadow
            d.draw_text(text, px, py, size, Color::WHITE);          // Text (always white for contrast)
        };
        
        draw_label(d, "NEURAL CORES", x + 10, y + 5, 10);
        
        // FPS display in top-right corner (with shadow)
        let fps_text = format!("FPS: {}", hw.target_fps);
        let fps_color = if hw.target_fps < 25 { Color::RED } 
                       else if hw.target_fps < 40 { Color::ORANGE } 
                       else { Color::GREEN };
        let fps_width = d.measure_text(&fps_text, 14);
        d.draw_text(&fps_text, x + w - fps_width - 9, y + 6, 14, Color::BLACK);  // Shadow
        d.draw_text(&fps_text, x + w - fps_width - 10, y + 5, 14, fps_color);
        ...
        // MEMORY MANIFOLD label with shadow
        d.draw_text("MEMORY MANIFOLD", x + 11, y + 141, 10, Color::BLACK);
        d.draw_text("MEMORY MANIFOLD", x + 10, y + 140, 10, Color::WHITE);
        let (cell, cols) = (12, 24);
    ...
    fn draw_terminal(&self, d: &mut RaylibDrawHandle, hw: &VirtualHardware, r: Rectangle) {
        ...
        let line_height = 15;
        let max_width = w - 20;
        let input_area_height = 35;  // Fixed area for input at bottom
        let content_height = h - input_area_height;
        
        let mut y_off = 10;
        
        // Draw kernel message history with wrapping (scrolling area)
        for msg in hw.kernel_messages.iter().take(15) {
            // Format JSON nicely if detected
            let formatted_msg = self.format_terminal_message(msg);
            let wrapped_lines = self.wrap_text(d, &formatted_msg, max_width, font_size);
            
            for line in wrapped_lines {
                if y_off > content_height - 10 { break; }
                
                // Color JSON keys differently
                let color = if line.trim().starts_with('"') && line.contains(':') {
                    Color::new(150, 200, 255, 255)  // JSON key color
                } else if line.contains("action") || line.contains("target") || line.contains("value") {
                    Color::new(200, 150, 255, 255)  // JSON field highlight
                } else {
                    self.theme.text_dim
                };
                
                d.draw_text(&line, x + 10, y + y_off, font_size, color);
                y_off += line_height;
            }
            if y_off > content_height - 10 { break; }
        }
        ...
        let input_y = y + h - input_area_height;
        
        // Separator line
        d.draw_line(x + 5, input_y, x + w - 5, input_y, self.theme.text_dim);
        
        // Draw prompt
        let prompt = "user@fugue-os:~$ ";
        d.draw_text(prompt, x + 10, input_y + 10, font_size, self.theme.accent);
        
        let prompt_width = d.measure_text(prompt, font_size);
        let text_start_x = x + 10 + prompt_width;
        
        // Draw shell buffer with cursor using inverted colors
        let buffer = &hw.shell_buffer;
        let cursor_pos = hw.shell_cursor.min(buffer.len());
        
        let mut char_x = text_start_x;
        for (i, ch) in buffer.chars().enumerate() {
            let ch_str = ch.to_string();
            let ch_width = d.measure_text(&ch_str, font_size);
            
            if i == cursor_pos {
                // Draw inverted cursor (white bg, black text)
                d.draw_rectangle(char_x, input_y + 8, ch_width + 2, font_size + 4, Color::WHITE);
                d.draw_text(&ch_str, char_x + 1, input_y + 10, font_size, Color::BLACK);
            } else {
                d.draw_text(&ch_str, char_x, input_y + 10, font_size, self.theme.text);
            }
            char_x += ch_width;
        }
        
        // Draw cursor at end if at end of buffer
        if cursor_pos >= buffer.len() {
            // Blinking cursor block
            if (d.get_time() * 2.0) as i32 % 2 == 0 {
                d.draw_rectangle(char_x, input_y + 8, 8, font_size + 4, Color::WHITE);
            }
        }
    }
    
    /// Format terminal messages - prettify JSON if detected
    fn format_terminal_message(&self, msg: &str) -> String {
        ...
        if let Some(json_start) = msg.find('{') {
            if let Some(json_end) = msg.rfind('}') {
                ...
                if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(json_str) {
                    let pretty = serde_json::to_string_pretty(&parsed).unwrap_or(json_str.to_string());
                    return format!("{}\n{}", prefix.trim(), pretty);
                }
            }
        }
...
/// Parse hex color - strictly uses first 6 valid hex chars, ignores extras
pub fn hex_to_color(hex: &str) -> Result<Color, ()> {
    ...
    let clean: String = hex.chars()
        ...
        .take(6)  // Strictly 6 characters max
        .collect();
